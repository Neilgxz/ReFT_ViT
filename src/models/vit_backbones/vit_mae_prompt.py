...
class PromptedVisionTransformer(VisionTransformer):
    def __init__(self, reft_cfg, prompt_cfg, **kwargs):
        super().__init__(reft_cfg, prompt_cfg=prompt_cfg, **kwargs)
        self.prompt_config = prompt_cfg
        self.reft_config = reft_cfg
...
def build_model(model_type, reft_cfg, prompt_cfg):
    if "vitb" in model_type:
        return vit_base_patch16(reft_cfg, prompt_cfg)
    elif "vitl" in model_type:
        return vit_large_patch16(reft_cfg, prompt_cfg)
    elif "vith" in model_type:
        return vit_huge_patch14(reft_cfg, prompt_cfg)


def vit_base_patch16(reft_cfg, prompt_cfg, **kwargs):
...
def vit_large_patch16(reft_cfg, prompt_cfg, **kwargs):
    model = PromptedVisionTransformer(
        reft_cfg, prompt_cfg,
        drop_path_rate=0.1, global_pool=True,  # using default settings for mae-finetune
...
def vit_huge_patch14(reft_cfg, prompt_cfg, **kwargs):
    model = PromptedVisionTransformer(
        reft_cfg, prompt_cfg,
        drop_path_rate=0.1, global_pool=True,  # using default settings for mae-finetune
